...
func render(maxIteration int, colors []color.RGBA, done chan struct{}) {
	...
	image := image.NewRGBA(image.Rectangle{image.Point{0, 0}, image.Point{width, height / 2}})

	for iy := 0; iy < height/2; iy++ { //Iteration over the 2 dimensional array defined as a 2d system coordinate and applying the mathematical calculation prior to generate the mandelbrot set
